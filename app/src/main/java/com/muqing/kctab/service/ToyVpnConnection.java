package com.muqing.kctab.service;
...
public class ToyVpnConnection implements Runnable {
    ...
    private ParcelFileDescriptor handshake(DatagramChannel tunnel)
        ...
        final int maxRetries = 5;      // 每个发送尝试次数
        final int sendDelayMs = 500;   // 重试间隔
        final long handshakeTimeoutMs = TimeUnit.SECONDS.toMillis(15); // 总超时
        ...
        while (!handshakeDone) {
            // 检查总超时
            if (System.currentTimeMillis() - startTime > handshakeTimeoutMs) {
            ...
            for (int i = 0; i < maxRetries; i++) {
                try {
                    packet.position(0);
                    tunnel.write(packet);
                    Log.i(getTag(), "Handshake packet sent (attempt " + (i + 1) + ")");
                } catch (PortUnreachableException e) {
                    Log.w(getTag(), "Handshake write failed, port unreachable, retrying...", e);
            ...
            try {
                ...
                if (length > 0 && packet.get(0) == 0) {
                    // 握手成功，返回配置好的 VPN 接口
                    String params = new String(packet.array(), 1, length - 1, US_ASCII).trim();
                    Log.i(getTag(), "Handshake response received: " + params);
                    return configure(params);
                }
            } catch (PortUnreachableException e) {
                Log.w(getTag(), "Handshake read failed, port unreachable, retrying...", e);
                Thread.sleep(sendDelayMs);
            }
        }
